...
export const fetchTimeSeriesData = async (): Promise<TimeSeriesResponse> => {
  ...
  const json: Record<string, Array<{ log_date: string; values: { value: number }[] }>> = await resp.json(); // TO CHANGE - we need to know the structure of the response

  const out: TimeSeriesResponse = {};
  for (const [key, arr] of Object.entries(json)) {
    if (!Array.isArray(arr)) continue;
    out[key] = arr
      .map(item => ({ x: item.log_date, y: item.values[0].value }));
  }
  return out;
};